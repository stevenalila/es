...
* actionsMerchant.delete
*	@description CRUD ACTION delete
*	@param ObjectId id - Id
*
* actionsMerchant.list
...
class MerchantList extends Component {
  ...
  // Load data on start
  componentWillMount() {
    this.props.actionsMerchant.loadMerchantList();
  }
  ...
  confirmDialogDelete(id) {
    this.props.actionsMerchant.deleteMerchant(this.state.idDelete).then(data => {
      this.props.actionsMerchant.loadMerchantList();
      this.setState({ openDialogDelete: false, idDelete: null });
    });
  }
            ...
            {this.props.list.map(row => (
              <TableRow key={row._id}>
                <TableCell component="th" scope="row">
                  <Link to={"/merchants/" + row._id} key={row._id}>
                    {row._id}
                  </Link>
                </TableCell>
...
function mapStateToProps(state, ownProps) {
  return {
    list: state.MerchantListReducer.listMerchant
  };
}
